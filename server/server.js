...
app.put('/api/members/:id', authenticateToken, authorize(['OWNER', 'ADMIN', 'STAFF', 'MEMBER']), async (req, res) => {
  const { id } = req.params;
...
app.get('/api/training-sessions', authenticateToken, authorize(['ADMIN', 'STAFF', 'TRAINER']), async (req, res) => {
  try {
    const sessions = await prisma.trainingSession.findMany({
      include: {
        member: true,
