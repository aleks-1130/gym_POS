...
export default function MemberDetail() {
    ...
    const submitRenew = async (paymentInfo = {}) => {
        try {
            await axios.post(`http://localhost:5000/api/members/${id}/renew`, {
                ...renewData,
                ...paymentInfo
            });
    ...
    const getMembershipProgress = () => {
        if (!member?.expiryDate || !member?.membershipPeriods?.length) return 0;
        const start = new Date(member.membershipPeriods[0].startDate);
    ...
    const getAttendanceScore = () => {
        ...
        const recentLogs = logs.filter(l => new Date(l.checkIn) >= thirtyDaysAgo);
        const count = recentLogs.length;
        if (count >= 20) return { label: 'Elite', color: 'emerald', icon: 'stars' };
        if (count >= 12) return { label: 'High', color: 'primary', icon: 'trending_up' };
        if (count >= 4) return { label: 'Moderate', color: 'orange', icon: 'remove' };
    ...
    const getLastActive = () => {
        ...
        return new Date(logs[0].checkIn).toLocaleDateString();
    };

    const getFilteredLogs = () => {
        let logs = member?.accessLogs || [];
        if (activityFilter === '7days') {
            const limit = new Date();
            limit.setDate(limit.getDate() - 7);
            logs = logs.filter(l => new Date(l.checkIn) >= limit);
    ...
    const getGroupedLogs = () => {
        ...
        const groups = {};
        logs.forEach(log => {
            const date = new Date(log.checkIn).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
    ...
    const activePeriods = (member.membershipPeriods || []).filter((p) => new Date(p.endDate) >= new Date());
    const combinedPlanLabel = [member.plan?.name, ...activePeriods.map((p) => p.plan?.name)].filter(Boolean).reduce((acc, name) => acc.includes(name) ? acc : [...acc, name], []).join(' + ') || 'No Plan';
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 items-start">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-${attendanceScore.color}-500/10 border border-${attendanceScore.color}-500/20 w-fit`}>
                                    <span className={`material-icons-round text-sm text-${attendanceScore.color}-400`}>{attendanceScore.icon}</span>
                                    <span className={`text-xs font-bold text-${attendanceScore.color}-400`}>{attendanceScore.label} Engagement</span>
                                </div>
                            </div>
                        </div>
                        <div className="flex flex-wrap gap-3 lg:ml-auto">
                            <button onClick={() => setShowPasswordModal(true)} className="bg-surfaceHighlight hover:bg-white/10 text-white px-6 py-3 rounded-xl font-medium flex items-center gap-2 border border-white/5 transition-all">
                                <span className="material-icons-round text-[18px]">lock_reset</span> Reset Password
                            </button>
                            <button onClick={() => setShowNotesModal(true)} className="bg-surfaceHighlight hover:bg-white/10 text-white px-6 py-3 rounded-xl font-medium flex items-center gap-2 border border-white/5 transition-all">
            ...
            <div className="bg-surface rounded-2xl border border-white/5 p-2 flex gap-2">
                {['overview', 'activity', 'payments', 'notes'].map(tab => (
                    <button
                        ...
                        className={`flex-1 px-6 py-3 rounded-xl font-bold transition-all capitalize ${activeTab === tab ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-text-muted hover:text-white hover:bg-white/5'}`}
                    >
                        {tab}
                    </button>
                ))}
                        ...
                        <div className="bg-gradient-to-br from-primary/10 to-orange-500/10 border border-primary/20 rounded-3xl p-6 space-y-4">
                            <div className="flex items-start justify-between">
                                ...
                                <div className="relative w-28 h-28">
                                    <svg className="transform -rotate-90 w-28 h-28">
                                        ...
                                        <circle cx="56" cy="56" r="48" stroke="currentColor" strokeWidth="8" fill="none" strokeDasharray={`${2 * Math.PI * 48}`} strokeDashoffset={`${2 * Math.PI * 48 * (1 - progress / 100)}`} className={`transition-all duration-1000 ${progress > 90 ? 'text-red-500' : 'text-primary'}`} strokeLinecap="round" />
                                    </svg>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                                    <div className={`h-full rounded-full transition-all duration-1000 ${progress > 90 ? 'bg-red-500' : 'bg-primary'}`} style={{ width: `${progress}%` }}></div>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/10">
                                <div className="bg-white/5 rounded-xl p-3">
                                    <p className="text-text-muted font-semibold text-xs mb-1">Start Date</p>
                                    <p className="text-white font-bold">{member.membershipPeriods?.[0]?.startDate ? new Date(member.membershipPeriods[0].startDate).toLocaleDateString() : 'N/A'}</p>
                                </div>
                                <div className="bg-white/5 rounded-xl p-3">
                                    <p className="text-text-muted font-semibold text-xs mb-1">Expiry Date</p>
                                    <p className={`font-bold ${isExpired ? 'text-red-400' : 'text-emerald-400'}`}>{member.expiryDate ? new Date(member.expiryDate).toLocaleDateString() : 'N/A'}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-surface rounded-3xl border border-white/5 overflow-hidden shadow-sm">
                            ...
                            <div className="p-5 space-y-4">
                                ...
                                <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 rounded-2xl p-4 border border-purple-500/20">
                                    ...
                                    <p className="text-2xl font-bold text-purple-400 mb-1">{isExpired ? 'High' : isExpiringSoon ? 'Medium' : 'Low'}</p>
                                    <p className="text-text-muted text-[10px]">{isExpired ? 'Membership expired' : isExpiringSoon ? 'Expiring soon' : 'Active membership'}</p>
                                </div>
                            </div>
                        </div>
            ...
            {showNotesModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-surface p-8 rounded-[32px] w-full max-w-md border border-white/10 shadow-2xl">
                        ...
                        <form onSubmit={async e => { e.preventDefault(); if (!noteData.trim()) return; try { await axios.post(`http://localhost:5000/api/members/${id}/notes`, { content: noteData.trim() }); setNoteData(''); setShowNotesModal(false); fetchNotes(); } catch (e) { alert("Failed to save note"); } }} className="space-y-4">
